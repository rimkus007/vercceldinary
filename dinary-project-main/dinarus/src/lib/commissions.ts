// Utilitaire pour calculer les commissions côté client
 ...
 */
export async function fetchCommissionRules(token: string, target: 'USER' | 'MERCHANT' = 'USER'): Promise<CommissionRule[]> {
  const now = Date.now();
